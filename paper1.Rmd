...
Here is the histogram of the most important variable: SCORE. It is defined as the overall score of buildings, which is the sum total of each item that was evaluated. 
```{r, include=FALSE}
library(readr)
...
```{r,}
data_clean %>% ggplot(aes(x= SCORE)) + geom_histogram(fill="blue", color="black") + theme_classic()+labs(x = "SCORE", title = "The overall score of buildings")
```
The histogram of the overall score of buildings shows that it is symmetric, with some extreme values on both sides. It shows a almost uniform distribution, and the center is around 75%. Overall, buildings in Toronto seem to be fine and things are not too bad.
...
```{r}
data_clean %>% ggplot(aes(y= RESULTS_OF_SCORE)) + geom_bar() + theme_classic()+labs(y = "Results of score", x = "Count",title = "The bar plot of the results of score")
```
The bar plot of the results of score shows that most buildings' evaluations are needed to be conducted in 2 years, which is consistent with the histigram above. Most buildings' scores are between 66 and 85 percent.
...
data_clean %>% filter(!is.na(PROPERTY_TYPE)) %>%
  ggplot(aes(y= RESULTS_OF_SCORE, fill = PROPERTY_TYPE)) +geom_bar() + theme_classic()+geom_text(stat = 'count',aes(label =..count.., vjust =-2))+
  labs(x = "Count", y = "Results of score", 
       title = "The bar plot of the results of score vs. Property type")
```
The bar plot of the variable RESULTS_OF_SCORE vs. PROPERTY_TYPE displays that most of the buildings are owned privately. Also, most private buildings' evaluations are needed to be conducted in 2 years.
...
```{r}
data_clean %>% ggplot(aes(x = SECURITY, y = SCORE,color = RESULTS_OF_SCORE))+geom_point()+theme_classic()+ labs(x = "conditions of the security system(s) in a building", y = "the overall score of the building", title = "The scatterplot of the relations between SCORE and SECURITY")
```
The scatter plot shows a linear relationship between the conditions of the security system(s) in a building and the verall score of the building. It illustrates that most buildings' evaluation are needed to be conducted in 1 or 2 years. 
